...
model=$1
local_batch_size=$2
profile_dir=${3:-mem_profile_data}
lbs_search_mode=${4:-dynamic}

if [ $# -lt 2 ]; then
  echo "[USAGE] [model] [Local Batch Size] [profile dir (default: mem_profile_data)] [LBS search mode (dynamic, static)]"
  exit 1
fi
